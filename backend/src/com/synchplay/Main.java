...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            // 好友推荐演示（组员算法：基于共同观看视频）
            System.out.println("\n【好友推荐 —— 基于共同观看视频】");
            if (!users.isEmpty()) {
                FriendRecommendation fr = new FriendRecommendation(graph);
                Node target = users.get(0);
                List<Node> recommendedFriends = fr.recommend(target.getNodeId());
                System.out.println("  目标用户: " + target.getNodeId());
                System.out.println("  推荐好友数: " + recommendedFriends.size());
                for (int i = 0; i < Math.min(10, recommendedFriends.size()); i++) {
                    Node f = recommendedFriends.get(i);
                    System.out.println("    " + (i + 1) + ". " + f.getNodeId() + " (" + f.getDisplayName() + ")");
                }
            }
