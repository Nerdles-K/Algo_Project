...
public class SynchPlayServer {
    ...
    /** 根路径：提供前端页面（方便开发调试，前后端仍可独立部署） */
    class RootHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange ex) throws IOException {
            String path = ex.getRequestURI().getPath();
            if (!path.equals("/")) { ex.sendResponseHeaders(404, -1); return; }
            java.nio.file.Path filePath = java.nio.file.Paths.get("frontend/index.html");
            if (java.nio.file.Files.exists(filePath)) {
                ...
                String msg = "{\"error\":\"frontend/index.html not found\"}";
